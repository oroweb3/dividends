...
async function main(){
 ...
 try{
  ...
  const exists=await c.query("select to_regclass('public.dividend_check_queue') is not null as present");
  if(!exists.rows[0].present)await c.query(readFileSync('supabase/migrations/202609180008_monitor_queue.sql','utf8'));
  await c.query('savepoint fixtures');
  await c.query(readFileSync('supabase/tests/monitor-queue.sql','utf8'));
  await c.query('rollback to savepoint fixtures');
  await c.query(apply?'commit':'rollback');
  console.log(apply?'Monitor queue migration applied; SQL tests passed and fixtures removed.':'Monitor queue SQL checks passed; migration and fixtures rolled back.');
 }finally{await c.query('rollback').catch(()=>{});await c.end();}
}
main().catch(e=>{console.error('Monitor queue checks failed; transaction rolled back. SQL code:',e.code??'connection',e.message?.startsWith('FAIL')?e.message:'');process.exitCode=1;});
