...
begin
 ...
 insert into public.dividend_verifications(tracking_id,event_id,event_version,status,evidence) values(t,suffix,1,'history-checked',jsonb_build_object('blockers','[]'::jsonb,'history',jsonb_build_object('status','no-activity-observed'),'current',jsonb_build_object('rawBaseUnits','1000'),'event',jsonb_build_object('effectiveTimeUtc',clock_timestamp()-interval '1 day','multiplierOld','1','multiplierNew','1.25'))) returning id into v;
 ...
 perform public.begin_dividend_signing(u,c);
